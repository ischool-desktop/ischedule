...
namespace ischedule
{
    public partial class frmWordReport : BaseForm
    {
        ...
        private MemoryStream mDefaultTemplateStream = new MemoryStream(Properties.Resources.功課表template);
        ...
        //列印項目
        private List<string> _printItems = new List<string>() { "教師", "科目", "註記", "班級", "場地", "鎖定", "教師代碼", "科目簡稱", "課程名稱" };
        //使用者決定的列印順序
        List<string> _printOrder = new List<string>();

        /// <summary>
        /// 無參數建構式
        /// </summary>
        public frmWordReport()
        {
            InitializeComponent();

            colPrintItem.Items.AddRange(_printItems.ToArray());
            ...
            lnkViewDefault.Click += (sender, e) =>
            {
                ...
                if (sfd.ShowDialog() == System.Windows.Forms.DialogResult.OK == true)
                {
                    try
                    {
                        DownloadTemplate(sfd, Properties.Resources.功課表template);
                    }
                    catch (Exception ex)
                    {
            ...
            lnkUploadCustom.Click += (sender, e) =>
            {
                ...
                ofd.Title = "上傳自訂功課表範本";
                ofd.Filter = "相容於 Word 2003 檔案 (*.doc)|*.doc";
        ...
        private void UploadTemplate(string fileName, ref bool uploadIndex, ref string uploadData)
        {
            ...
            MemoryStream testms = new MemoryStream();
            try
            {
                Aspose.Words.Document doc = new Aspose.Words.Document(ms);
                doc.Save(testms, SaveFormat.Doc);
                testms = null;
            }
            catch
            {
                throw new Exception("此範本限用相容於 Word 2003 檔案。");
            }
        ...
        /// <param name="e"></param>
        private void frmWordReport_Load(object sender, EventArgs e)
        {
            chkOdd.Checked = true;
            ...
            {
                case "Teacher":
                    MyTools.SetDefaultRow(dgv, "班級", "科目");
                    ...
                    break;
                case "Class":
                    MyTools.SetDefaultRow(dgv, "教師", "科目");
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataGridViewRow row in dgv.Rows)
            {
                ...
                string item = row.Cells[colPrintItem.Index].Value + "";

                if (!_printOrder.Contains(item))
                    _printOrder.Add(item);
            }
        ...
        private void Print()
        {
            ...
            DataTable MergeData = MyTools.InitDataTable();

            #region 迴圈在此
            for (int i = 1; i <= AssocIDs.Count; i++)
            {
                DataRow Row = MergeData.NewRow();
                ...
                switch (MainFormBL.Instance.GetSelectedType())
                {
                    case "Teacher":
                        ...
                        Dictionary<string, DayPeriod> tevents = GroupByToEvents(apsCur);

                        foreach (DayPeriod evtCur in tevents.Values)
                        {
                            string Subject = evtCur.Subject;
                            string SubjectAlias = evtCur.SubjectAlias;

                            //有群組課程的話，使用群組當 Key。
                            string key = Subject + "," + (string.IsNullOrWhiteSpace(evtCur.Group) ? evtCur.ClassName : evtCur.Group);
                            ...
                            SubjectCount sc = dicSubject[key];
                            foreach (CEvent e in evtCur.Events)
                            {
                                CheckAddResource(sc, e.DisplayClassName.Trim());
                                ...
                                else if (e.Comment.ToUpper().Equals("F"))
                                    CounselingLength += e.Length;
                                else
                                    BasicLength += e.Length;
                            }
                        }
                    ...
                    case "Class":
                        ...
                        Dictionary<string, DayPeriod> cevents = GroupByToEvents(apsCur);

                        foreach (DayPeriod evtCur in cevents.Values)
                        {
                            string Subject = evtCur.Subject;
                            string SubjectAlias = evtCur.SubjectAlias;
                            ...
                            foreach (CEvent e in evtCur.Events)
                            {
                                CheckAddResource(sc, e.TeacherID1);
                                CheckAddResource(sc, e.TeacherID2);
                                CheckAddResource(sc, e.TeacherID3);
                            }
                        }
                    ...
                    case "Classroom":
                        ...
                        Dictionary<string, DayPeriod> crevents = GroupByToEvents(apsCur);

                        foreach (DayPeriod evtCur in crevents.Values)
                        {
                            string Subject = evtCur.Subject;
                            string SubjectAlias = evtCur.SubjectAlias;
                ...
                Row["學年度"] = "" + schLocal.SchoolYear;
                Row["學期"] = "" + schLocal.Semester;
                Row["資源名稱"] = "" + LPViewName;
                Row["資源類別"] = "" + LPViewType;
                Row["班導師姓名"] = "" + LPViewClassTeacherName;
                ...
                Row["教師基本時數"] = "" + BasicLength;
                Row["教師兼課時數"] = "" + ExtraLength;
                Row["教師輔導時數"] = "" + CounselingLength;
                Row["教師授課時數"] = "" + TotalLength;
                ...
                //將子table的資料整理到大table中
                int row_index = 0;
                foreach (DataRow r in tabScheduleDetail.Rows)
                {
                    row_index++;
                    foreach (DataColumn col in tabScheduleDetail.Columns)
                    {
                        string colName = col.ColumnName;
                        int day = int.TryParse(colName, out day) ? day : 0;

                        string key = string.Empty;
                        string value = r[colName] + "";
                        ...
                        switch (colName)
                        {
                            ...
                            case "Time":
                                ...
                                break;
                            default:
                                key = MyTools.GetDay(day) + "_" + row_index;
                                break;
                        }
            ...
            try
            {
                ...
                {
                    doc.Save(sfd.FileName, SaveFormat.Doc);
                    System.Diagnostics.Process.Start(sfd.FileName);
                }
        ...
        private DataTable SubjectSummaryOrderbyResource(SortedDictionary<string, SubjectCount> dicSubject)
        {
            ...
            foreach (SubjectCount vSubjectCount in dicSubject.Values.OrderByDescending(y => y.GetResources()).Reverse())
            {
                ...
                tabSubject.Rows.Add(
                    ...
                    _printOrder.Contains("科目簡稱") ? vSubjectCount.SubjectAlias : vSubjectCount.Subject,
                    vSubjectCount.Len,
        ...
        private static void CheckAddResource(SubjectCount sc, string rc)
        {
            ...
            {
                if (!sc.Resources.Contains(rc))
                    sc.Resources.Add(rc);
            }
        }

        private Dictionary<string, DayPeriod> GroupByToEvents(Appointments apts)
        {
            //群組出 Event 清單，因為數個  Appointment 可能對應到同一個 Event。
            Dictionary<string, DayPeriod> events = new Dictionary<string, DayPeriod>();
            foreach (Appointment apt in apts)
            {
                if (string.IsNullOrWhiteSpace(apt.EventID))
                    continue;

                CEvent evtCur = schLocal.CEvents[apt.EventID];

                DayPeriod dp = new DayPeriod(evtCur.WeekDay, evtCur.PeriodNo);

                if (!events.ContainsKey(dp.Key))
                    events.Add(dp.Key, dp);

                events[dp.Key].Events.Add(evtCur);
            }
            return events;
        }
        ...
        class DayPeriod
        {
            public DayPeriod(int week, int period)
            {
                Key = string.Format("{0}:{1}", week, period);
                Events = new List<CEvent>();
            }

            public string Key { get; private set; }

            public List<CEvent> Events { get; private set; }

            public string Subject { get { return Events[0].DisplaySubjectName.Trim(); } }

            public string SubjectAlias { get { return Events[0].SubjectAlias.Trim(); } }

            public string ClassName { get { return Events[0].DisplayClassName.Trim(); } }

            public int Length { get { return Events[0].Length; } }

            public string Group { get { return Events[0].CourseGroup.Trim(); } }
        }
        ...
        {
            //辨識單雙週的ID
            string odd = schLocal.CEvents[EventIDS].WeekFlag == 1 ? EventIDS : EventIDD;
            string even = schLocal.CEvents[EventIDD].WeekFlag == 2 ? EventIDD : EventIDS;

            string strEvtInfo = string.Empty;

            //單週列印
            if (chkOdd.Checked)
                strEvtInfo = strEvtInfo.AppendStr(GetEventDisplayStr(odd));

            //雙週列印
            if (chkEven.Checked)
                strEvtInfo = strEvtInfo.AppendStr(GetEventDisplayStr(even));
        ...
        private string GetEventDisplayStr(string EventID)
        {
            ...
            bool isLock = false;
            foreach (string item in _printOrder)
            {
                switch (item)
                {
                    case "教師":
                        bool mustPrintCode = _printOrder.Contains("教師代碼");
                        strEvtInfo = strEvtInfo.AppendStr(eventLocal.GetTeacherString(mustPrintCode));
                        break;

                    case "班級":
                        strEvtInfo = strEvtInfo.AppendStr(schLocal.Classes[eventLocal.ClassID].Name);
                        break;

                    case "場地":
                        if (!schLocal.Classrooms[eventLocal.ClassroomID].Name.Equals("無"))
                            strEvtInfo = strEvtInfo.AppendStr(schLocal.Classrooms[eventLocal.ClassroomID].Name);
                        break;

                    case "科目":
                        strEvtInfo = strEvtInfo.AppendStr(schLocal.Subjects[eventLocal.SubjectID].Name + eventLocal.WeekFlag.GetWeekFlagStr());
                        break;

                    case "科目簡稱":
                        strEvtInfo = strEvtInfo.AppendStr(eventLocal.SubjectAlias + eventLocal.WeekFlag.GetWeekFlagStr());
                        break;

                    case "課程名稱":
                        strEvtInfo = strEvtInfo.AppendStr(eventLocal.CourseName + eventLocal.WeekFlag.GetWeekFlagStr());
                        break;

                    case "鎖定":
                        isLock = eventLocal.ManualLock;
                        break;

                    case "註記":
                        strEvtInfo = strEvtInfo.AppendStr(eventLocal.Comment);
                        break;
                }
            }
            ...
            if (isLock)
                strEvtInfo = strEvtInfo.Insert(0, "*");
    ...
    {
        public static string AppendStr(this string str, string append)
        {
            //如果要str的字串不是空的就必須先換行
        ...
        public static void SetDefaultRow(DataGridView dgv, params string[] str)
        {
            dgv.Rows.Clear();
        ...
        public static DataTable InitDataTable()
        {
            ...
            table.Columns.Add("資源名稱");
            table.Columns.Add("資源類別");
            table.Columns.Add("班導師姓名");
            table.Columns.Add("教師基本時數");
            table.Columns.Add("教師兼課時數");
            table.Columns.Add("教師輔導時數");
            table.Columns.Add("教師授課時數");
            table.Columns.Add("教師代碼");
            table.Columns.Add("教師專長");
            table.Columns.Add("教師註解");
            table.Columns.Add("列印日期");
            table.Columns.Add("總節數");

            for (int i = 1; i <= 20; i++)
            {
                table.Columns.Add("節次" + i);
                table.Columns.Add("時間" + i);

                for (int day = 1; day <= 7;day++)
                    table.Columns.Add(GetDay(day) + "_" +  i);
            }
